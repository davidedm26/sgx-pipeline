...
from config.settings import MAX_RETRIES, BACKOFF_FACTOR, REQUEST_TIMEOUT, COMPANYLIST_URL , CORPORATEINFO_URL , PAGE_SIZE, MAX_PAGES, MAX_WORKERS
import requests
...
def get_company_result_dict():
    ...
    results = get_search_results(periodstart="20051030_160000", periodend="20251028_155959", url=CORPORATEINFO_URL, pagesize=PAGE_SIZE)
    ...
    for page_num in range(min(n_pages, MAX_PAGES) if MAX_PAGES > 0 else n_pages):
        ...
        print(f"Processing page {page_num + 1}/{n_pages} (page size {PAGE_SIZE}) for company search")
        response = get_search_results(periodstart="20051030_160000", periodend="20251028_155959", url=CORPORATEINFO_URL, pagesize=PAGE_SIZE, pagestart=page_num)
